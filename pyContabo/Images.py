...
class Images:
    ...
    def get(self, id: str = None, page: int = None, pageSize: int = None, orderByFields: str = None,
            orderBy: str = None, name: str = None, standardImage: bool = None):
        ...
        if id:
            ...
            url = f"https://api.contabo.com/v1/compute/images?{f'page={page}&' if page is not None else ''}{f'size={pageSize}&' if pageSize is not None else ''}{f'orderBy={orderByFields}:{orderBy}&' if orderByFields is not None else ''}{f'name={name}&' if name is not None else ''}{f'standardImage={standardImage}&' if standardImage is not None else ''}"
