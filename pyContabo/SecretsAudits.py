...
class SecretsAudits:
    ...
    def get(self, page=None, pageSize=None, orderBy=None, secretId=None,
        ...
        url = f"https://api.contabo.com/v1/secrets/audits?{f'page={page}&' if page is not None else ''}{f'size={pageSize}&' if pageSize is not None else ''}{f'orderBy={orderBy}&' if orderBy is not None else ''}{f'instanceId={instanceId}&' if instanceId is not None else ''}{f'requestId={requestId}&' if requestId is not None else ''}{f'instanceId={secretId}&' if secretId is not None else ''}{f'changedBy={changedBy}&' if changedBy is not None else ''}"
        url = url[:-1]
